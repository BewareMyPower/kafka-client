...
inline std::string __strip(const std::string& s) {
    ...
    size_t pos_begin = 0;
    while (pos_begin < s.size() && isblank(s[pos_begin]))
        ++pos_begin;
    if (pos_begin == s.size() || s[pos_begin] == '#')
        return "";

    size_t pos_end = s.size();
    while (pos_end > pos_begin && isblank(s[pos_end - 1]))
        --pos_end;
    // s[pos_begin, pos_end)
...
inline std::pair<GlobalConf, TopicConf> readConfig(const std::string& filename) {
    ...
    while (std::getline(fin, line)) {
        ...
        if (type == NONE) {
            error::Exit("[file: %s] line %d doesn't have a header like "
